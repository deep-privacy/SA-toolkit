...
if [ "$(hostname)" == "lst" ]; then
    ...
    do
        sbatch -p gpu -c 40 --gres gpu:rtx8000:3 -N 1 --mem 180G --constraint=noexcl --time 7:30:00 -o .log_parrallel_tdnnf_vq_nonorm_addlayer_$dim.out --job-name tdnnf_vq_$dim --wrap="bash ./local/train_all.sh $dim"
    done
    for dim in 48 64 128
    do
        sbatch -p gpu -c 40 --gres gpu:rtx6000:3 -N 1 --mem 180G --constraint=noexcl --time 7:30:00 -o .log_parrallel_tdnnf_vq_nonorm_addlayer_$dim.out --job-name tdnnf_vq_$dim --wrap="bash ./local/train_all.sh $dim"
    done
