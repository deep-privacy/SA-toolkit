...
pat1  = re.compile("\s*Transition-state.*phone\s+=\s+(.*)\s+hmm-state\s+=\s+(.*)\s+forward.*")
    ...
    for line in lines:
        ...
        if y is not None:
            ...
            if curr_phoneme == "SIL":
                ...
                trans2state[ti] = curr_phoneme
            if len(trans2state[ti]) < 2:
                trans2state[ti] += " "

    #print(trans2state)
    with open(args.output, "w") as fo:
        for key in input_trs:
            fo.write(key+"len_"+str(len(input_trs[key])) + " ")
