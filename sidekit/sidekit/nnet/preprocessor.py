...
from .bn_preprocessor import get_extract_bn
...
class BNFrontEnd(torch.nn.Module):
    """
    Module that compute the bottleneck representation from another network (ASR-bn for example)
    """
    def __init__(self):
        super(BNFrontEnd, self).__init__()
    ...
    def forward(self, x, is_eval=False):
        ...
        :return:
        """
        if self.extract_bn == None: # first init set device!
            self.extract_bn = get_extract_bn(device=x.device)

        with torch.no_grad():
            with torch.cuda.amp.autocast(enabled=False):
                return self.extract_bn(x, is_eval)
