...
class OnlineNaturalGradient:
    ...
    def _precondition_directions_internal(self, X, initial_product):
        ...
        U = U[:, eigvalues_sorted.indices].to(X.device)
        ...
        sqrt_c_t = eigvalues.pow(0.5).to(X.device)
        rho_t1 = (
            (1.0)
            / (D - R)
            * (
    ...
    def _compute_Wt1(self, N, d_t1, rho_t1, U, sqrt_c_t, inv_sqrt_et, J_t):
        ...
        w_t_coeff = ((1.0 - eta) / (eta / N) * (d_t + rho_t)).to(W_t.device)
        # this is in CPU
        A_t = (
            U.T * ((eta / N) * sqrt_e_t1 * inv_sqrt_c_t)[:, None] * inv_sqrt_et[None, :]
        )
        A_t = A_t.to(W_t.device)
    ...
    def _reorthogonalize_Rt1(self, d_t1, rho_t1, W_t1, temp_O):
        ...
        try:
            ...
            Omat_inv = Omat_inv.to(self.W_t.device)
        ...
        if not cholesky_ok:
            ...
            W_t1.copy_(W_t1_cpu.to(self.W_t.device))
            W_t1.mul_(sqrt_e_t1.to(self.W_t.device)[:, None])
            return
    ...
    @torch.no_grad()
    def precondition_directions(self, X, device="cuda:0"):
