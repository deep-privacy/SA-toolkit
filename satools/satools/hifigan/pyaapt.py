...
--------------------------------------------
"""
def yaapt(_in:torch.Tensor, kwargs):
    out = []
    max_length = 0

    device = _in.device  # Preserve the original device
    _in = _in.to("cpu")  # Move input to CPU for processing

    # Collect results and determine the maximum length
    for i in range(_in.size(0)):
        result = torch.tensor(
            _yaapt(_in[i:i + 1, :], kwargs).samp_values,
            dtype=torch.float32
        )
        out.append(result)
        max_length = max(max_length, result.size(0))

    # Pad all results to the maximum length
    padded_out = torch.zeros((len(out), max_length), dtype=torch.float32)
    for idx, tensor in enumerate(out):
        padded_out[idx, :tensor.size(0)] = tensor

    return padded_out.to(device)


def _yaapt(signal, kwargs):
    ...
    signal = SignalObj(signal.squeeze().numpy(), 16000)
...
def crs_corr(data, lag_min, lag_max):
    ...
    x = np.sqrt(formula_denom)
    if x.any() == 0:
        phi[lag_min:lag_max] = formula_nume
    else:
        phi[lag_min:lag_max] = formula_nume/x
