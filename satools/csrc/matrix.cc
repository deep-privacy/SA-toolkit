...
kaldi::CuSubMatrix<kaldi::BaseFloat> TensorToKaldiCuSubMatrix(torch::Tensor &t) {
    if(kaldi::CuDevice::Instantiate().Enabled()) {
        if(t.is_contiguous())  {
            ...
            std::cout << "pkwrap kaldi wrapper: ERROR: Cannot convert to CuSubMatrix because tensor is not contiguous" << std::endl;
            exit(1);
        }
    }
    else {
        std::cout << "pkwrap kaldi wrapper: ERROR: Cannot convert to CuSubMatrix because GPU is not enabled (kaldi.InstantiateKaldiCuda() not called but required for model in training mode)" << std::endl;
        exit(1);
    }
