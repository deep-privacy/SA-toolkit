...
class CodeDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        audio_pkwrap = audio_pkwrap[:code_length * self.code_hop_size]
        assert self.vqvae or audio.shape[0] // self.code_hop_size == code.shape[0], "Code audio mismatch"
        assert self.vqvae or audio_pkwrap.shape[0] // self.code_hop_size == code.shape[0], "Code audio_pkwrap mismatch"

        while audio.shape[0] < self.segment_size:
            audio = np.hstack([audio, audio])
            audio_pkwrap = np.hstack([audio_pkwrap, audio_pkwrap])
            if not self.vqvae:
        ...
        if self.vqvae:
            ...
            if self.segment_size != -1:
                audio, audio_pkwrap, code = self._sample_interval([audio, audio_pkwrap, code])
        ...
        if self.vqvae:
            ...
            feats = {"code": torch.tensor(code.squeeze())}
        ...
        if self.f0_normalize:
            ...
            if self.f0_feats:
                feats['f0_stats'] = torch.FloatTensor([mean, std]).view(-1)

        audio_pkwrap = torch.tensor(audio_pkwrap, dtype=torch.float32, requires_grad=False).unsqueeze(0)
        feats['audio_pkwrap'] = audio_pkwrap
    ...
    def _get_spkr(self, idx):
        ...
        spkr_id = torch.LongTensor([self.spkr_to_id[spkr_name]]).view(1)
        return spkr_id
