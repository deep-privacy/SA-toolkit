...
if [ ! -f $mark ]; then
  ...
  pip3 install torch==$torch_version+cu$cuda_version_witout_dot torchvision==$torchvision_version+cu$cuda_version_witout_dot torchaudio==$torchaudio_version -f $torch_wheels || echo "Failed to find pytorch for cuda '$cuda_version', use please specify another version with: ./install.sh '11.1'" && exit 1
  cd $home
  touch $mark
fi
